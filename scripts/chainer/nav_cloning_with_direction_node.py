...
#        self.pose_sub = rospy.Subscriber("/amcl_pose", PoseWithCovarianceStamped, self.callback_pose)
        ...
        # self.load_path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/model_with_dir_'+str(self.mode)+'/20221130_05_05_41_10000/model.net'
        self.load_path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/model_with_dir_'+str(self.mode)+'/20221203_20:51:23_20000_model6/model.net'
...
#            self.dl.save(self.save_path)
        ...
        if self.learning:
            ...
            if self.mode == "manual":
                ...
                angle_error = abs(action - target_action)
                loss = 0
                times = 1 if (cmd_dir == np.array([100, 0, 0])).all() else 7 #9
                if angle_error > 0.05 and abs(self.action) <= 0.45:
                    action, loss = self.dl.act_and_trains(imgobj, cmd_dir, target_action, times)
                    action = action * 1.5
                    action = max(min(action, 0.45), -0.45) # abs 0.6
                ...
                # if (cmd_dir == np.array([100, 0, 0])).all():
                #     pass
                # else:
                #     self.select_dl = False
